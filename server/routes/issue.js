...
const IssueData = mongoose.models.IssueData || mongoose.model('IssueData', IssueDataSchema);
...
// /issue API - Buyer app sends issue request
router.post('/', async (req, res) => {
	try {
		const payload = req.body;
		
		console.log('=== INCOMING ISSUE REQUEST ===');
		...
		// Store issue data in MongoDB Atlas with retry mechanism
		let retries = 0;
		const maxRetries = 3;
		...
		while (retries < maxRetries) {
			try {
				...
				console.log('✅ Issue data saved to MongoDB Atlas database');
				console.log('📊 Saved issue request for transaction:', context.transaction_id);
				break; // Exit loop if successful
			} catch (dbError) {
				retries++;
				console.error(`❌ Failed to save issue data to MongoDB Atlas (Attempt ${retries}/${maxRetries}):`, dbError.message);
				if (retries >= maxRetries) {
					console.error('❌ Max retries reached. Could not save issue data.');
				} else {
		...
		console.log('✅ Sending ACK response for issue request');
...
// Debug endpoint to view stored issue requests
router.get('/debug', async (req, res) => {
	try {
		const limit = parseInt(req.query.limit) || 50;
		const issueRequests = await IssueData.find().sort({ created_at: -1 }).limit(limit);
